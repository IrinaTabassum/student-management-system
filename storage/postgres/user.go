...
func (s PostgresStorage) CreateClass( u storage.Class) error {
	...
	if u.ID == 0 {
		return fmt.Errorf("unable to insert user into db")
	}
	return nil

}

const listClassQuery = `SELECT * FROM classes WHERE deleted_at IS NULL;`

func (s PostgresStorage) ListOfClass() ([]storage.Class, error) {
...
func (s PostgresStorage) CreateSubject( u storage.Subject) error {
...
const getClassByClassnameQuery = `SELECT * FROM classes WHERE class_name=$1 AND deleted_at IS NULL`

func (s PostgresStorage) GetClassByClassName(classname string) (*storage.Class, error) {
	var c storage.Class
	if err := s.DB.Get(&c, getClassByClassnameQuery, classname); err != nil {
		log.Println(err)
		return nil, err
	}
